...
class ModelEMA(object):
    ...
    def __init__(self, args, model, decay):
        ...
        self.ema.to(args.device) # move the copy to the device specified in the arguments
        self.ema.eval() # set the copy to evaluation mode
        self.decay = decay # store the decay factor
        self.ema_has_module = hasattr(self.ema, 'module') # check if the original model has a module
    ...
    def update(self, model):
        needs_module = hasattr(model, 'module') and not self.ema_has_module # check if the model has a module
        with torch.no_grad(): # temporarily set requires_grad to False
            ...
            for k in self.param_keys:
                ...
                esd[k].copy_(ema_v * self.decay + (1. - self.decay) * model_v) # update the value in the EMA
